def batch_input_validation_node(state: dict) -> dict:
    ...
    검증 항목:
      1. technology_analysis 완료 여부 + 필수 필드
      2. market_policy_analysis 완료 여부 + 필수 필드
      3. competition_analysis 완료 또는 skip 여부
      4. company_profile 필수 필드
    """
    errors: list[str] = []
    ...
    # 3. competition_analysis (완료 or skip)
    comp = cs.get("competition_analysis", {})
    if not comp or not isinstance(comp, dict):
        errors.append("competition_analysis 누락")
    elif "analyzed" not in comp:
        errors.append("competition_analysis.analyzed 필드 누락")

    # 4. company_profile 필수 필드
    if not profile.get("company_name"):
        errors.append("company_profile.company_name 누락")
