...
에너지 전환(Energy Transition) 도메인 스타트업의 투자 가능성을 자동 평가하는 Multi-Agent 시스템입니다.
...
초기 설계 시 Supervisor 패턴을 검토했으나, 채택하지 않았습니다.

- 투자 평가 파이프라인은 **순서가 명확**합니다 — 탐색 → 도메인 확인 → 분석 → 판단 → 보고서. Supervisor가 동적으로 라우팅할 필요가 없음
- Supervisor를 두면 서브 에이전트가 다른 서브 에이전트를 호출하는 구조가 생기기 쉬운데, 이는 디버깅과 상태 추적을 어렵게 만듦
- 대신 **LangGraph의 조건부 라우팅**으로 분기를 처리 — 도메인 부적합 시 즉시 기각, 평가 지표 누락 시 재평가 등
- 기술/시장 분석처럼 독립적인 에이전트는 **Fan-out 병렬 실행**으로 처리

```
[Startup Search] → [Domain Check] → 부적합 시 즉시 기각 (0점)
                                   ↓ 적합
                    ┌───────────────┴───────────────┐
              [Tech Analysis]              [Market & Policy]   ← 병렬 + Corrective RAG
              [+ CRAG]                     [+ CRAG]
                    └───────────────┬───────────────┘
                        [Competitor Analysis]  ← 웹 검색
                                   ↓
                      [Investment Decision]   ← 9항목 가중합 100점
                           ↓            ↺ 평가 지표 누락 시 재평가
                       [Evaluation Check]
                                   ↓
                        [Report Writer]       ← Markdown → HTML → PDF
```
...
| **Report Writer** | 투자 보고서 생성 | - | → Markdown + PDF 보고서 |

---

## 2. Corrective RAG Pipeline
...
<!-- 📸 사진: CRAG 파이프라인 다이어그램 -->
<!-- 경로: GitHub 업로드 이미지 (기존 CRAG 파이프라인) -->
...
RAG 검색 품질은 임베딩 모델에 의존합니다. 도메인 문서가 한영 혼합(기술 용어는 영어, 정책·시장 자료는 한국어)이라 두 모델을 15개 도메인 질문으로 비교했습니다.
...
- **LLM Judge 관련성**: 대부분 동등하거나 bge-m3이 소폭 우위
- **Top-5 Jaccard 겹침**: 평균 0.50 — 두 모델이 절반은 같은 문서를 가져오지만, 나머지 절반에서 bge-m3이 더 관련성 높은 문서를 검색
- **결론**: 한국어 파인튜닝 모델이 반드시 유리하지 않음. 도메인 문서가 한영 혼합이라 다국어 범용 모델(bge-m3)이 더 적합 → **BAAI/bge-m3 채택**
...
## 4. Evaluation Criteria — VC + CVC 관점
...
| # | 기준 | 배점 | 출처 | 핵심 질문 |
|---|------|------|------|----------|
| 1 | **SK 사업 시너지** | **15점** | **SK 전략자료 + CVC 커스텀** | SK 에너지 밸류체인과 직접 연결되는가? |
...
## 5. Two Modes — Single & Batch

시스템은 두 가지 모드를 제공합니다. 각 모드가 보여주는 것이 다릅니다.
...
| **보여주는 것** | "이 기업이 왜 투자할 만한가"에 대한 근거 | "여러 기업 중 어떤 기업이 투자 가치가 있는가"에 대한 스크리닝 |

---

## 6. Results — Single Mode

### 스탠다드에너지 — 투자 판정 (81.35/100)
...
투자 판정 근거: VIB 기술 기반 ESS 기업. TRL 7 수준의 상용화 직전 기술력, Series C 1,225억 원 유치, 대전 구암역 실증 사례 보유. 기술적 차별성과 시장 성장 가능성이 높아 투자 가치가 있으나, 원자재 공급과 생산 비용의 변동성은 리스크 요소.
...
## 7. Results — Batch Mode

12개 스타트업을 자동 탐색·평가한 결과. **1개 invest, 11개 reject.**
...
**invest 1개 — 이것이 정상인 이유:**
- 실제 VC 투자 심사에서도 스크리닝 단계 통과율은 극히 낮으며, 대부분 기각되는 것이 현실
- PoC 단계에서 12개라는 제한된 표본으로 검증. 대규모 배치 시 invest 비율은 달라질 수 있음
- 점수 인플레이션 방지 로직(역방향 웹 검색 + 균일 감점)이 보수적으로 작동 → 시스템 신뢰도 확보

**스탠다드에너지만 통과한 근거:**
- 기술차별성 10점(최고점) — RAG 문서의 기술 벤치마크와 대조 시 VIB 기술(TRL 7, 에너지 효율 97%+, 수명 20년+)의 우위가 입증
...
- "온실가스 측정"은 환경/탄소 모니터링 영역으로, 에너지를 생산·저장·전환하는 기업이 아님
- 도메인 체크 에이전트가 에너지 밸류체인 외의 기업을 정확히 걸러냄

### 항목별 횡단 분석

| 항목 | 배점 | 평균 | 득점율 | 비고 |
|------|------|------|--------|------|
| 시장 규모·성장성 | 17점 | 10.2 | 60% | 가장 높은 득점율 — 에너지 시장 자체의 성장성 반영 |
| 기술 차별성 | 14점 | 8.0 | 57% | 기술력 있는 스타트업이 많으나 상용화 수준에서 차이 |
...
### 8-1. RAGAS — RAG 파이프라인 품질 평가

15개 도메인 질문(시장/정책 7개, 기술 분석 8개)에 대해 RAGAS 4대 메트릭을 측정했습니다.

| Metric | Score | 의미 |
|--------|-------|------|
| **Faithfulness** | **0.9611** | 응답이 검색 문서에 근거하는 정도 — 환각 최소 |
| **Response Relevancy** | **0.8371** | 응답이 질문에 적절한 정도 |
| **Context Precision** | **0.9869** | 검색된 문서 중 실제 관련 문서 비율 |
| **Context Recall** | **1.0000** | 정답에 필요한 정보를 검색이 커버하는 정도 |

- Context Recall 1.0 → 필요한 정보를 빠짐없이 검색
- Faithfulness 0.96 → LLM이 검색 문서에 충실하게 답변, 환각 최소화
- Context Precision 0.99 → 검색 결과 대부분이 실제 관련 문서

<!-- 📸 사진: RAGAS 평가 결과 차트 -->
<!-- 경로: GitHub 업로드 이미지 (기존 RAGAS 스크린샷 2개) -->

### 8-2. Ablation Study — RAG ON vs OFF
...
- 스탠다드에너지(VRFB, TRL 6-8)만 유일하게 invest → RAG 문서의 기술 벤치마크가 정당한 가점의 근거로 작용
- **RAG = "점수를 높이는 도구"가 아니라 "변별력을 만드는 도구"**

<!-- 📸 사진: Ablation 비교 상세 차트 -->
<!-- 경로: outputs/ablation_results/ablation_comparison_detailed.png -->

### 8-3. 시스템 일관성 검증

세 가지 평가 모드에서 동일한 결론이 도출되었습니다.

| 평가 모드 | 스탠다드에너지 | 나머지 기업 |
|----------|-------------|----------|
...
3개 모드에서 스탠다드에너지만 일관되게 invest. RAG가 꺼지면 어떤 기업도 통과하지 못함 → 시스템이 무작위가 아닌, RAG 근거에 기반한 재현 가능한 판단을 내리고 있음을 입증.
